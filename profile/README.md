...
1. **Тестовый режим** — пользователь взаимодействует напрямую с моделью.  
2. **RPC-режим** — пользователь обращается к модели через `nginx`, `bot-test-gateway` и `RabbitMQ`.  
3. **Полный режим** — пользователь работает через `frontend`, `backend`, очередь сообщений и поисковый движок `OpenSearch`.  

## 🔹 Демонстрация Полный режима
